...
ep = [0.01 0.1 0.5 1 5 10];

MI =[];
every=[];

parfor n = 1: length(ep)
t = [];
...
for rep = 1 : 10
   % Data are seperated into Train part (80%) and Test part (20%)
    ...
    [B, constant] = example(Test, Train, ep(n));
    B=B'; 
    X=Train(:,1:end-1); 
    Y=Train(:,end); 
...
%tp->true postivie, tn->true negative, fn->false negative, fp->false
%positve. fnr->false negative rate, fpr->false positve rate
   [tp, tn, fn, fp] = deal(0);
    for i = 1: size(Y,1)
       if(s(i) == 1 && Y(i) == 1) 
         tp = tp+1; 
